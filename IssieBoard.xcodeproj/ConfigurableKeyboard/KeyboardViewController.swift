...
class KeyboardViewController: UIInputViewController {
    ...
    */
            ...
            for rowKeys in page.rows { // TODO: quick hack
                for key in rowKeys {
                    if let keyView = self.layout?.viewForKey(key) {
                      ...
                      //  case Key.KeyType.CtrlZ :
                        //    keyView.addTarget(self, action: Selector("ctrlZTapped:"), forControlEvents: .TouchDown)
                        //case Key.KeyType.AltCtrlZ :
                         //   keyView.addTarget(self, action: Selector("altCtrlZTapped:"), forControlEvents: .TouchDown)
    ...
    //Orit: these constraints are causing an error - needs to be fixed
 ...
 //       }
    ...
    func keyPressedHelper(sender: KeyboardKey) {
        if let model = self.layout?.keyForView(sender) {
            ...
            //else if model.uppercaseOutput/*lowercaseOutput*/ == "'" {
             //   self.currentMode = 0
            //}
...
//        }
   ...
   /* func ctrlZTapped (sender : KeyboardKey) {
         assert(true, "hi")
    }
    
    func altCtrlZTapped (sender : KeyboardKey) {
        assert(true,"hi")
    }*/
...
//
    ...
    //fix this one - whn dismis keyboard is pressed
    func keyPressed(key: Key) {
        if let proxy = (self.textDocumentProxy as? UIKeyInput) {
            proxy.insertText(key.getKeyOutput())//outputForCase(/*shiftState.uppercase()*/))
        }
    }
